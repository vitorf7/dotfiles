...
EOF
	...
	arch-chroot /mnt pacman -S \
		...
		wayland hyprland hyprpaper hyprlock rofi-wayland waybar \
		...
		nvidia nvidia-settings nvidia-utils nvidia-lts lib32-nvidia-utils nvidia-prime \
		hyprshot hyprpicker \
		swaync avizo volumectl lightctl \
		cliphist \
		...
		kitty firefox go egl-wayland nodejs yarn npm rust cargo fastfetch fd eza starship
...
install_bootloader() {
	...
	arch-chroot /mnt sed -i "s/^GRUB_CMDLINE_LINUX_DEFAULT.*/GRUB_CMDLINE_LINUX_DEFAULT=\"loglevel=3 quiet cryptdevice=UUID=$ROOT_UUID:$LUKS_NAME root=\/dev\/mapper\/$LUKS_NAME nvidia-drm.modeset=1 nvidia.NVreg_PreserveVideoMemoryAllocations=1\"/" /etc/default/grub
...
install_aur_packages() {
	...
	arch-chroot /mnt sudo -u $USERNAME yay -S --noconfirm ghostty zen-browser-bin vicinae matugen awww-daemon spotify-launcher easyeffects
...
setup_gaming_essentials() {
	...
	# Core Wine and compatibility layer
	arch-chroot /mnt pacman -S --noconfirm wine wine-gecko wine-mono winetricks umu-launcher
	...
	arch-chroot /mnt pacman -S --noconfirm \
		...
		lib32-nvidia-utils lib32-openal \
		lib32-libpulse lib32-alsa-plugins \
		lib32-gnutls lib32-giflib lib32-libpng \
		lib32-libldap lib32-gnutls lib32-mpg123 \
		lib32-libxml2 lib32-openal lib32-v4l-utils \
		lib32-libxslt lib32-dbus
	...
	# Install Protontricks via AUR
	print_message "$BLUE" "Installing Protontricks via AUR"
	arch-chroot /mnt sudo -u $USERNAME yay -S --noconfirm protontricks

	print_message "$GREEN" "Core gaming packages installed..."

	# Flatpak gaming apps
	print_message "$BLUE" "Installing Flatpak gaming applications..."
	arch-chroot /mnt flatpak install -y flathub \
		com.usebottles.bottles \
		com.heroicgameslauncher.hgl

	print_message "$GREEN" "Flatpak gaming apps installed..."

	# AUR gaming packages
	print_message "$BLUE" "Installing AUR gaming packages..."
...
setup_gaming_performance() {
	print_message "$BLUE" "Configuring gaming performance optimizations..."

	# Increase vm.max_map_count for game compatibility (SteamOS value)
	cat >/mnt/etc/sysctl.d/80-gamecompatibility.conf <<EOF
# Increase memory map count for gaming compatibility
# This value matches SteamOS default and is required for many modern games
vm.max_map_count = 2147483642
EOF

	print_message "$GREEN" "Gaming performance optimizations configured..."
}
