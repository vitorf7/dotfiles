{...}: {
  ...
  flake.modules.homeManager.git = {
    ...
    workTemplatePath =
      if gitCfg.work.enable
      then config.sops.templates."git-identity-work".path
      else "";
    defaultInclude = [
      {
        path =
          if gitCfg.defaultProfile == "work"
          then workTemplatePath
          else personalTemplatePath;
      }
    ];
    personalIncludes =
      lib.optionals (gitCfg.personal.enable && gitCfg.defaultProfile != "personal")
      (map (dir: {
    ...
    workIncludes =
      lib.optionals (gitCfg.work.enable && gitCfg.defaultProfile != "work")
      (map (dir: {
          condition = "gitdir:${dir}";
