...
`nix flake update` only bumps the flake `inputs` in `flake.nix` — it does **not** touch the hand-pinned derivations under `pkgs/` (`strongbox`, `wiresteward`, `go-latest`, `tide-island`, `mouseless`). Those pin a `rev`/`tag`/`version` + `hash` by hand and only move when someone refreshes them.
