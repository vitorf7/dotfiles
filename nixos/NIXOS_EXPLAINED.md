...
`modules/hosts/uw-thinkpad-x1/_hardware-configuration.nix` is a third variant: a real, checked-in file that deliberately declares no `fileSystems`, scaffolded before the physical laptop existed. It lets `.#uw-thinkpad-x1` *evaluate* (so `nix flake check` and `nix eval` work from any machine) but not *build* — `nixos-rebuild build` fails loudly until `scripts/initial_nixos_setup.sh` overwrites it with the real generated config on install day.
